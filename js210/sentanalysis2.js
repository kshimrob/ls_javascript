...
function sentiment(text) {
  var sentiment;
  text = text.toLowerCase().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g,'').split(/\s/);

  var positiveMatches = text.filter(function(word) {
    return word.match(positiveRegex);
  });

  var negativeMatches = text.filter(word => word.match(negativeRegex));
  ...
  console.log('The sentiment of this text is ' + sentiment + '.');
}
